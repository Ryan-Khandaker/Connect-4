...
#player input
def Playerinput(board, score_count):
  #  global score_count
    global current_player
    inp = int(input('Insert an integer from 1-7: '))
    n = score_count[inp]
    if inp>= 1 and inp<=7 and board[6-n][inp-1] == '-':
        board[6-n][inp-1]=current_player
        score_count[inp]+=1
    else:
        print('Invalid entry')
        swich_player()
...
#switching players
def swich_player(current_player):
 #   global current_player
    if current_player == 'X':
        current_player = 'O'
...
while game_running:
    ...
    boardd(board2)
    Playerinput(board2, score_count)
    win_diag(board2)
    win_horizontal(board2)
    win_vertical(board2)
    tie(board2)
    swich_player(current_player)
    if not game_running:
        boardd(board2)